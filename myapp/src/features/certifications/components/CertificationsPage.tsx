...
export default function CertificationsPage() {
  ...
  const filtered = useMemo(() => {
    return certifications.filter((c) => {
      ...
      if (search && !c.title.toLowerCase().includes(search.toLowerCase()) &&
          ...
          !c.skills.some(s => s.toLowerCase().includes(search.toLowerCase()))) return false;
      return true;
    });
  }, [activeType, activeProvider, activeLevel, search]);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Hero with type tabs */}
      <CertHero activeType={activeType} onTypeChange={setActiveType} />

      {/* Partners strip */}
      <FeaturedPartners />

      {/* Provider filter */}
      <ProviderFilter active={activeProvider} onChange={setActiveProvider} />
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {activeType !== "all" && (
              <span className="flex items-center gap-1 bg-amber-50 text-amber-700 text-xs font-medium px-3 py-1 rounded-full border border-amber-200">
                {activeType}
                <button onClick={() => setActiveType("all")} className="ml-1 hover:text-amber-900 text-base leading-none">×</button>
              </span>
            )}
            {activeProvider !== "all" && (
              <span className="flex items-center gap-1 bg-blue-50 text-blue-700 text-xs font-medium px-3 py-1 rounded-full border border-blue-200">
                {activeProvider}
                <button onClick={() => setActiveProvider("all")} className="ml-1 hover:text-blue-900 text-base leading-none">×</button>
              </span>
            )}
            {activeLevel !== "all" && (
              <span className="flex items-center gap-1 bg-green-50 text-green-700 text-xs font-medium px-3 py-1 rounded-full border border-green-200">
                {activeLevel}
                <button onClick={() => setActiveLevel("all")} className="ml-1 hover:text-green-900 text-base leading-none">×</button>
              </span>
            )}
            {search && (
              <span className="flex items-center gap-1 bg-gray-100 text-gray-700 text-xs font-medium px-3 py-1 rounded-full">
                &quot;{search}&quot;
                <button onClick={() => setSearch("")} className="ml-1 hover:text-gray-900 text-base leading-none">×</button>
              </span>
            )}
            <button
